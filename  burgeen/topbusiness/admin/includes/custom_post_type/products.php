...
function meta_options(){
	...
	$max_sum = $custom['postsum_ajax'][0] ? $custom['postsum_ajax'][0] : 1;
		...
		for ( $i=1; $i<=$max_sum; $i++ ) :
			 ...
			 * 3. From there, append the wordpress folder to modified relative image path to get the absolute image path
			 * 4. With ths abs. img path, check if the file exists
			 * 5. If it exists, we will update the ${'i'.$i} variable with the new URL
			*/
			$relImgPath = str_replace(site_url(), "",${'i'.$i}); // remote the http://<domain> portion from the path
