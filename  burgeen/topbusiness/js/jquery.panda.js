...
//				pslides.css({height: 50}).delay(300).animate({height: h}, 750); // <-- This line is wrong, it begins with height of 50
				pslides.css({height: h}).delay(300).animate({height: h}, 750);	// <-- This line is correct, it begins straightaway with line height of the first slider image
		...
		function next() {
			...
			function placeholderHeight() {
				...
				var newH = nextSlide.height();
				var sliderClass = parent.attr("class");
				var reMatch = (/ps-\d+/).exec(sliderClass);

				if (reMatch instanceof Array && typeof(reMatch[0]) == "string") {
					var sliderId = reMatch[0];
					var selector = "div." + sliderId + "> div.pslides";

					f(selector, parent).animate({height: newH}, 750);
				} else {
					f('.pslides',parent).animate({height: newH}, 750);
				}
...
//					newSlide.addClass('block').css({opacity: 0, top: 0, left: 0}).animate({opacity: 1}, 200);
					...
					if (parent.hasClass("ptd")) {
						// Hide active slide
						f('.pslides',parent).children('.block').animate({opacity: 0}, 300, function(){ f(this).removeClass('block').css('opacity', 1); });

						// Show new slide
						newSlide.addClass('block').css({opacity: 0}).addClass('block').animate({opacity: 1}, 200);
					} 
					else 
					{
						// Hide active slide
