...
function slider_meta_options(){
	...
	$max_sum_spec = $custom['specsum_ajax'][0];
		...
		for( $i=1; $i<=$max_sum_spec; $i++ ) :
			...
			/*
			 * Need to further sanitize ${'s-img'.$i}
			 * 1. Get the image path relative to the wordpress folder
			 * 2. Check to see if this image path has a trailing "-nnnxnnn" before the extension, for e.g. "06-300x158.gif" or "04-300x158-150x158.jpg"
			 *    If it does, remove the -nnnxnnn just before '.ext'
			 * 2. From there, append the wordpress folder to modified relative image path to get the absolute image path
			 * 3. With ths abs. img path, check if the file exists
			 * 4. If it exists, we will update the ${'s-img'.$i} variable with the new URL
			 */
			$relImgPath = str_replace(site_url(), "",${'s-img'.$i}); // remote the http://<domain> portion from the path
			$relImgPath = preg_replace('/(.+)-\d{3}x\d{3}\.([a-z0-9]+)$/', '$1.$2', $relImgPath);
			$absImgPath = ABSPATH . $relImgPath;
			$absImgPath = str_replace('//', '/', $absImgPath);
			if (file_exists($absImgPath)) {
				${'s-img'.$i} = site_url() . $relImgPath;
			}
											...
											<table class="options-table">
												...
												<tr>
													...
													<td>
														...
														<a class="thickbox button select-image clone-select" href="media-upload.php?post_id=<?php echo the_ID() ?>&type=image&TB_iframe=true&width=640&height=469"><?php _e('Browse image','pandathemes') ?></a>
														...
														<table class="options-table">
															...
															<tr>
																...
																<td>
																	...
																	<a class="thickbox button select-image clone-select" id="<?php echo $i ?>" title="<?php echo $i; ?>" href="media-upload.php?post_id=<?php echo the_ID() ?>&type=image&TB_iframe=true&width=640&height=469"><?php _e('Browse image','pandathemes') ?></a>
		...
		<script type='text/javascript'>
			...
			k(document).ready(function() {
				...
				k('.select-image').click(function() {
					id = k(this).attr('id');
					tb_show('', 'media-upload.php?type=image&TB_iframe=true&width=640&height=469');
				...
				window.send_to_editor = function(html) {
					...
					k('#thumb-img'+id).attr({
						src: function() {
							...
							/*
							 * (Added by Wen Yen)
							 * When user chooses any image size other than the "Full Size" size, a "nnnxnnn" will be appended to "imgurl" (see above),
							 * which then is again appended with -150x150 and assigned to "path".
							 * This will result in a "path" equal <image filename>-nnnxnnn-150x150.<ext>.
							 * But when the image was uploaded, a thumbnail version is created as <image filename>-150x150.<ext>,
							 * so "path" now points to a non-existent file.
							 * To solve this, we remove any -nnnxnnn just before -150x150.
							 */   
							//path = path.replace(/-\d+x\d+-150x150/, '-150x150');
							path = path.replace(/-\d+x\d+-150x150(\.\w{3,4})/, '-150x150$1');
