<div class="item it2">
	...
	// T H U M B N A I L
		...
		if ($feat_img) :
			...
			// Display image
			if ($src) : $out = '<a href="' . get_permalink($post->ID) . '"><img class="t2 fr br3" src="' . get_bloginfo('template_url') . '/timthumb.php?src=' . $path . '&amp;h=' . $h . '&amp;w=' . $w . '&amp;zc=1&amp;q=90'. $a . '" width="' . $w . '" alt="' . get_the_title($post->ID) . '" /></a>'; else : $out = ''; endif;
			
		echo $out;

	?><h3><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a><?php edit_post_link( __( 'edit','pandathemes' ), '<span class="f13"> - ', '</span>' ) ?></h3><?php
